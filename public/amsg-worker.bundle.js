...
// node_modules/.pnpm/@rei-standard+amsg-shared@0.4.0-next.2/node_modules/@rei-standard/amsg-shared/dist/index.mjs
...
// node_modules/.pnpm/@rei-standard+amsg-server@2.6.0-next.10/node_modules/@rei-standard/amsg-server/dist/chunk-ME6UG54W.mjs
...
function validateLlmMessagesArray(messages) {
  const err2 = validateLlmMessagesShape(messages);
  if (!err2) return null;
  const { code, index: i, toolCallIndex: j } = err2;
  switch (code) {
...
function createUpdateMessageHandler(ctx) {
  async function PUT(url, headers, body) {
    ...
    if (updates.pushSubscription !== void 0 && updates.pushSubscription !== null && typeof updates.pushSubscription !== "object") {
...
var SERVER_VERSION = true ? "2.6.0-next.10" : "0.0.0-dev";
...
var pendingSelfLogs = /* @__PURE__ */ new Map();
var PENDING_SELF_LOG_TTL_MS = 15 * 6e4;
var prunePendingSelfLogs = (nowMs) => {
...
var amsgAfterSend = async (info) => {
  ...
  const text = entry.texts.slice(0, sentCount).filter((message) => message.trim()).join("\n");
